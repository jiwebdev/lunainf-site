...
try {
 ...
 for(const width of [1440,390]) {
  ...
  for(const state of ['rest','cognition','memory','inference']) {
   ...
   await (await page.$('.overview-brain')).screenshot({path:`.tmp/routing-${width}-${state}.png`});
  ...
  if(width===1440) {
   const collisions=await page.evaluate(()=>{
    ...
    return [...document.querySelectorAll('[data-bundle-source]')].flatMap(group=>{
     const path=group.querySelector('.ribbon-halo');const matrix=path.getScreenCTM();const pad=Number(path.getAttribute('stroke-width'))*matrix.a/2+2;const hits=new Set();
