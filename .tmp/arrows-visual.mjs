...
try {
 ...
 for(const width of [1440,390]) {
  ...
  assert.equal(await page.$$eval('.ribbon-arrow',els=>els.length),14);
  assert.equal(await page.$$eval('.ribbon-strand,.ribbon-halo,.ribbon-chevron',els=>els.length),0);
  for(const state of ['rest','cognition','memory','inference']) {
   ...
   await new Promise(resolve=>setTimeout(resolve,220));
   await (await page.$('.overview-brain')).screenshot({path:`.tmp/arrows-${width}-${state}.png`});
  ...
  if(width===1440) {
   const collisions=await page.evaluate(()=>{
    ...
    return [...document.querySelectorAll('[data-bundle-source]')].flatMap(group=>{
     const path=group.querySelector('.ribbon-arrow');const matrix=path.getScreenCTM();const pad=2;const hits=new Set();
